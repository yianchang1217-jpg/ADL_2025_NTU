...
def main():
    ...
    # ======【插入點 A】放在 logger.info("***** Running training *****") 之後 ======
    metrics_csv_path = os.path.join(args.output_dir, "metrics_per_epoch.csv")
    # 建立 CSV 檔頭（若不存在）
    if accelerator.is_main_process:
        os.makedirs(args.output_dir, exist_ok=True)
        if not os.path.exists(metrics_csv_path):
            with open(metrics_csv_path, "w", encoding="utf-8") as f:
                f.write("epoch,train_loss,eval_exact,eval_f1\n")
    
    best_score = float("-inf")   # 若你有「保存最佳模型」需求，這裡一併初始化
    best_epoch = -1
    # ======【插入點 A】結束 ======
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        print(f"Epoch {epoch} evaluation metrics: {eval_metric}")
        if args.with_tracking:
            accelerator.log(
                {"eval_exact": eval_metric["exact_match"], "eval_f1": eval_metric["f1"], "epoch": epoch},
                step=completed_steps,
            )
        # ======【插入點 B】放在「每個 epoch 的 evaluation 完成後」這段的正下方 ======
        # 你原本有：
        # eval_metric = metric.compute(predictions=prediction.predictions, references=prediction.label_ids)
        # logger.info(f"Epoch {epoch} evaluation metrics: {eval_metric}")
        # print(f"Epoch {epoch} evaluation metrics: {eval_metric}")
        # if args.with_tracking: accelerator.log(...)
        
        # 1) 計算本 epoch 的平均訓練損失（total_loss 在上面迴圈已經加總）
        if args.with_tracking:
            # active_dataloader 就是本 epoch 的訓練 dataloader（上面已經設好）
            epoch_train_loss = (total_loss.item() / len(active_dataloader)) if len(active_dataloader) > 0 else float("nan")
        else:
            # 若沒開 tracking，也盡量不要報錯
            epoch_train_loss = float("nan")
        
        # 2) 取出 EM / F1
        em = float(eval_metric.get("exact_match", float("nan")))
        f1 = float(eval_metric.get("f1", float("nan")))
        
        # 3) 追加寫入 CSV（單機只讓 main process 寫）
        if accelerator.is_main_process:
            with open(metrics_csv_path, "a", encoding="utf-8") as f:
                f.write(f"{epoch},{epoch_train_loss},{em},{f1}\n")
        
        # 4) （選配）同時保留最佳模型（以 EM 當指標；改成 f1 也可）
        current_score = em  # 想用 F1 就改成 f1
        if current_score > best_score:
            best_score = current_score
            best_epoch = epoch
            accelerator.print(f"[Best Model Updated] epoch={epoch}, score={current_score:.4f}")
            ...
            # 存一份當前指標
            with open(os.path.join(save_dir, "best_metrics.json"), "w", encoding="utf-8") as f:
                json.dump({"best_epoch": best_epoch, "best_score": best_score,
                           "metric": "exact_match", "eval_metric": eval_metric},
    ...
    outputs_numpy = (start_logits_concat, end_logits_concat)
    prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy)
    # print("len(predictions) =", len(prediction.predictions))
    # print("len(references)  =", len(prediction.label_ids))
    # print("sample prediction:", prediction.predictions[0])
    # print("sample reference :", prediction.label_ids[0])
    eval_metric = metric.compute(predictions=prediction.predictions, references=prediction.label_ids)
    ...
    try:
        import winsound
        winsound.Beep(1000, 700)  # 高頻短音
        winsound.Beep(600, 500)   # 低頻短音
        print("訓練完成！已發出提示音")
    except Exception as e:
        from IPython.display import Audio, display
        sound = Audio(data=[0, 1]*4000, rate=8000, autoplay=True)
        display(sound)
        print("訓練完成！（用 IPython Audio 播放提示音）")
    # ==============================
